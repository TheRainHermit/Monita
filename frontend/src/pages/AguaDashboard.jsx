...
import {
  ...
  useAguaAnomalies,
  useAguaComparativa,
  useAguaZonasDistinct,
  useAguaTiposUsuarioDistinct
} from "../hooks/useAguaApi";
...
export default function AguaDashboard() {
  ...
  const tiposUsuarioDisponibles = useAguaTiposUsuarioDistinct();
  ...
  const { data: summary, loading: loadingSummary } = useAguaSummary(filtros);
  const { data: series, loading: loadingSeries } = useAguaTimeSeries(filtros);
  const { data: zonas, loading: loadingZonas } = useAguaZonas(filtros);
  const { data: anomalies, loading: loadingAnomalies } = useAguaAnomalies(filtros);
  const { data: comparativa, loading: loadingComparativa } = useAguaComparativa({
    zona1,
    zona2,
    tipo_usuario: tipoUsuario,
    fecha_inicio: fechaInicio,
  ...
  return (
    <motion.div
      ...
      aria-label="Dashboard de monitoreo de agua"
    >
      <h1>
        <span data-tip="Panel principal para el monitoreo y análisis de consumo de agua" tabIndex={0}>
          Monitoreo de Agua
        </span>
      </h1>
      <FiltersPanel
